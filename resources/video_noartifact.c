...
int
main() {
	int fullcolor = 0xFFFF; // initial full display color for single buffer
	// make current frame all white
    solid_color(vp->fbp, 0xFFFF); 

	// Initialize the backbuffer 
    vp->bfbp = (struct fb_t  *volatile) backbuffer.pixels; // backbuffer
	// back buffer all red
	solid_color(vp->bfbp, 0xF800);
    
	
   
    // press and release button 0 to alternate between the two	
	while (1) {
		ButtonMaskWait(0x3); // wait for button 0 or 1
		fbswap();
	}
}
