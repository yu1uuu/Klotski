...
bool get_highlighted(struct Box *boxes, int box_id) {
    for (int i = 0; i < num_boxes; ++i) {
        if (boxes[i].id == box_id) {
...
void move_two_grid_box_horizontal(int box_id){
    ...
    if(left_pressed && col >= 1) {
        ...
        if(col >= 1 && grid_array[row - 1][col + 1] == 0) {
            erase_two_grid_box_horizontal(fbp, row, col);
            draw_two_grid_box_horizontal(fbp, row, col+1, color);
            set_col(game, box_id, col+1);
        }
...
int main() {
    ...
    //Move a box according to Which botton is pressed

    /*
    move_box(select_id);
    */

    //Left shift selected box
    ...
    */
    ...
    //Test code in Cpulator
	
	//Move box 9 right wards and back
	...
	//Move box 7 left wards and back
	...
	//Move box 9 left wards and back
	...
	////Move box 6 down wards and back
	...
	////Move box 5 right wards and back
